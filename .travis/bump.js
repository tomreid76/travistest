...
if (!isPullRequest) {
  ...
  if (PREVIOUS_VERSION === CURRENT_VERSION) {
    ...
    cp.exec('npm version patch --no-git-tag-version', { encoding: 'utf8' }, (error, stdout, stderr) => {
      if (error) {
      ...
      cp.execSync(`git add . && git commit -m "Travis Autobump to ${NEW_VERSION} [skip ci]"`);
      cp.execSync(`git push origin master`);
      cp.execSync(`git tag -a ${NEW_VERSION} -m "Travis Autobump to ${NEW_VERSION} [skip ci]"`);
